...
class _TableCalendarState extends State<TableCalendar>
  ...
  Widget _buildTable() {
    ...
    while (x < widget.calendarController._visibleDays.value.length) {
      var days = widget.calendarController._visibleDays.value
          .skip(x)
          .take(widget.calendarController.numberOfDaysToShow)
          .toList();

      /// remove weekend days from list if set to true
      if (widget.excludeWeekends) {
        days.removeWhere((element) =>
        element.weekday == DateTime.saturday ||
            element.weekday == DateTime.sunday);
      }

      ///check if all the days in the list are from previous month
      ///and if so then we remove them because it caused an extra row
      ///to be rendered with no data inside
      int outsideDaysCount = 0;
      days.forEach((date) {
      ...
      if (days.length != outsideDaysCount) {
        children.add(_buildTableRow(days));
      }

      x += widget.calendarController.numberOfDaysToShow;
    }

    return Table(
      // Makes this Table fill its parent horizontally
      defaultColumnWidth: FractionColumnWidth(
          1.0 / widget.calendarController.numberOfDaysToShow),
      children: children,
    );
  }
  ...
  TableRow _buildDaysOfWeek() {
    return TableRow(
      ...
      children: widget.calendarController._visibleDays.value
          .take(widget.calendarController.numberOfDaysToShow)
          .map((date) {
